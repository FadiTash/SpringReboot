...
export function Controller(path?: string, middleware?: MiddleWare) {
    return function (target: any) {
        ...
        for (let o in controllerRegistry) {
            ...
            router[entry.method](entry.route, (req, resp, next) => {
                if (entry.middleware) {
                    entry.middleware(req, resp, () => {
                        entry.value(req, resp);
                    });
                } else {
                    entry.value(req, resp);
                }
...
export function Request(method: RequestMethod, route: string, middleware?: MiddleWare) {
...
export function Get(route: string, middleware?: MiddleWare) {
    return Request(RequestMethod.Get, route, middleware);
}

export function Post(route: string, middleware?: MiddleWare) {
    return Request(RequestMethod.Post, route, middleware);
}
